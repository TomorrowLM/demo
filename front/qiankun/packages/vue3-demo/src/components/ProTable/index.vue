...
<template>
  ...
  <div class="card table-main">
    ...
    >
      ...
      <!-- 无数据 -->
      <template #empty>
        <div class="table-empty flex-center flex-column">
          <slot name="empty">
            <!-- <img src="@/assets/images/dustRemove/no-data.png" alt="notData" /> -->
            <svg-icon width="100px" height="100px" icon-class="data_search_empty"></svg-icon>
            <span>暂无数据</span>
          </slot>
        </div>
      </template>
    ...
    <slot name="pagination">
      <Pagination
        class="float-right d-flex mt-16"
        v-if="pagination"
        :pageable="pageable"
...
const props = withDefaults(defineProps<ProTableProps>(), {
  ...
  toolButton: true,
  rowKey: 'id',
  searchCol: () => ({ xs: 4, sm: 4, md: 4, lg: 4, xl: 4 })
})
...
const setEnumMap = async ({ prop, enum: enumValue }: ColumnProps) => {
  ...
  // 判断data是否为字符串数组
  if (_.map(data, (item) => typeof item).every((item) => item === 'string')) {
    data.forEach((item, index) => {
      data[index] = { key: item, value: item }
    })
  }
