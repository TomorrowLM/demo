...
exports.axios = exports.VueAxios = void 0;
const vue_1 = __importDefault(require("vue"));
const axios_1 = __importDefault(require("axios"));
const vant_1 = require("vant");
const axios_2 = require("../../../packages/vue-demo-mobile/src/utils/axios");
//单例模式，即同一时间只会存在一个 Toast
vant_1.Toast.allowMultiple();
...
const handleParams = (config) => {
    console.log(config);
    const token = vue_1.default.ls.get('token');
    const { method } = config;
    if (token) {
...
const installer = {
    vm: {},
    install(Vue) {
        Vue.use(axios_2.VueAxios, service);
    }
};
