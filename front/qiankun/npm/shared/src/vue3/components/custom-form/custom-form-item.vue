<template>
  <template v-if="splitEle.length === 1">
    <el-input v-if="item.type === 'input'" v-model="formData[splitEle[0]]" :placeholder="`请输入${item.label}`" style="width: 100%" />
    <el-input-number
    ...
    ></el-input-number>
    <el-input v-if="item.type === 'input-suffix'" :disabled="item.disabled" v-model="formData[splitEle[0]]" :placeholder="`请输入${item.label}`" :label-width="item.labelWidth">
    ...
    />
    <el-select v-else-if="item.type === 'select'" :disabled="item.disabled" v-model="formData[splitEle[0]]" :placeholder="`请选择${item.label}`" style="width: 100%">
    ...
    <div v-else-if="item.type === 'DateTimePicker'">
      <el-date-picker format="YYYY-MM-DD HH:mm:ss" :disabled="item.disabled" v-model="formData[splitEle[0]]" type="datetime" placeholder="选择日期时间"> </el-date-picker>
    </div>
  </template>
  <!-- {{ splitEle.length }} -->
  <template v-if="splitEle.length > 1">
    <!-- splitEle.slice 不会改变 -->
    <!-- {{ splitEle.slice(1) }} -->
    <custom-form-item v-if="formData && formData.hasOwnProperty(splitEle[0])" v-model:formData="formData[splitEle[0]]" :item="item" :splitEle="splitEle.slice(1)"></custom-form-item>
  </template>
</template>
<script lang="ts">
export default {
