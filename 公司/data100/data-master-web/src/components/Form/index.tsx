...
const GeneralForm: React.FC<GeneralFormProps> = (props) => {
  ...
  return (
    <ProForm
      ...
      submitter={{
        render: (_, dom): any => { return showFooter ? <FooterToolbar style={{ position: 'absolute',bottom:'-12px' }}>{dom}</FooterToolbar> : null },
      }}
      onFinish={(values) => {
        if (!isSubmit) {
          onFinish(values);
      ...
      <Row gutter={[16, 0]} className={styles.slsForm}>
        {
          columns.map((item: GeneralFormColumnsItem): any => {
            ...
            const { name, siteName = [], type, label, tip, placeholder, options = [], rules = [], siteRules = [], fieldNames, colSpan = 12, colXs = 12, disabled = false, precision = 0, mode, showSearch = true, showArrow = true, optionFilterProp = 'label', treeNodeFilterProp = 'title', layout = 'vertical' } = item
            ...
            const ColumnsData = {
              ...
              'INPUT_SITE': <Form.Item key={name} label={label}>
                <Input.Group compact>
                  <Form.Item className={styles.siteInputItem} name={siteName[0]} rules={siteRules[0]}><Input style={{ width: 120, textAlign: 'center' }} placeholder="请输入" suffix="分" /></Form.Item>
                  <Input
              ...
              'INPUT': <ProFormText key={name} name={name} label={label} tooltip={tip} placeholder={placeholder} rules={rules} fieldProps={ { disabled:name==='password', type: mode || 'text', allowClear: true, onPressEnter: (e) => onPressEnter(e), defaultValue:name==='password'?'111111':'' } } />,
            ...
            return <Col key={`col${name}`} lg={colSpan} xs={colXs}>{ ColumnsData[type] }</Col>
          })
        }
        { children }
      </Row>
