...
import { queryDownloadCenterList, deleteDownloadCenterList, clearDownloadCenterList } from '@/services/user';
...
const DownloadCenter: React.FC<DownloadCenterProps> = () => {
  ...
  const getDownloadCenterList = async () => {
    ...
    const data = current === 1 ? result.data.records : [...list, ...result.data.records];
  ...
  const removeList = async (id: Number) => {
    await deleteDownloadCenterList(id);
    message.success('删除成功');
    refresh();
  }

  const clearList = async () => {
    await clearDownloadCenterList();
