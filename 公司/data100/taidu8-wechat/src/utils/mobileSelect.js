...
export function cityArr() {
  ...
  for (var b = 0; b < 61; b++) {
    ...
    obj6.id = t + "";
    obj6.value = str;
    obj6.childs = timeArrs;
    weekdayArr.push(obj6);
  }
  return weekdayArr;
}
...
export function MobileSelect(config) {
  this.mobileSelect;
...
MobileSelect.prototype = {
  ...
  renderWheels: function (wheelsData, cancelBtnText, ensureBtnText) {
    ...
    for (var i = 0; i < wheelsData.length; i++) {
      ...
      if (_this.jsonType) {
        for (var b = 0; b < wheelsData[i].data.length; b++) {
          tempHTML +=
            '<li data-id="' +
            wheelsData[i].data[b][_this.keyMap.id] +
            '">' +
            wheelsData[i].data[b][_this.keyMap.value] +
            "</li>";
        }
      } else {
        for (var k = 0; k < wheelsData[i].data.length; k++) {
          tempHTML += "<li>" + wheelsData[i].data[k] + "</li>";
        }
      }
      tempHTML += "</ul></div>";
    }
  ...
  checkRange: function (index, posIndexArr) {
    ...
    for (var l = 0; l <= index; l++) {
      if (l == 0) {
        ...
        resultNode = resultNode[_this.keyMap.childs][posIndexArr[l]];
      }
    }
  ...
  resetPosition: function (index, posIndexArr) {
    ...
    if (_this.slider.length > posIndexArr.length) {
      ...
      if (_this.slider.length < posIndexArr.length) {
        ...
        for (var m = 0; m < tempCount; m++) {
          tempPosArr.pop();
        }
      }
    }
    for (var n = index + 1; n < tempPosArr.length; n++) {
      tempPosArr[i] = 0;
    }
    return tempPosArr;
  },
  reRenderWheels: function () {
    ...
    for (var i = 0; i < _this.displayJson.length; i++) {
      (function (i) {
        var tempHTML = "";
        if (_this.wheel[i]) {
          for (var c = 0; c < _this.displayJson[i].length; c++) {
            tempHTML +=
              '<li data-id="' +
              _this.displayJson[i][c][_this.keyMap.id] +
              '">' +
              _this.displayJson[i][c][_this.keyMap.value] +
              "</li>";
          }
    ...
    if (_this.cascade) {
      ...
      if (_this.jsonType) {
        for (var d = 0; d < data.length; d++) {
          tempHTML +=
            '<li data-id="' +
            data[d][_this.keyMap.id] +
            '">' +
            data[d][_this.keyMap.value] +
            "</li>";
        }
  ...
  getCurValue: function () {
    ...
    if (_this.cascade) {
      ...
      if (_this.jsonType) {
        for (var f = 0; f < _this.curDistance.length; f++) {
          temp.push(
            _this.wheelsData[f].data[_this.getIndex(_this.curDistance[f])]
          );
        }
      } else {
        for (var g = 0; g < _this.curDistance.length; g++) {
          temp.push(_this.getInnerHtml(g));
        }
      }
    }
    return temp;
