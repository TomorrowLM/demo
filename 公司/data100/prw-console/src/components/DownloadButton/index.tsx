...
const DownloadButton: React.FC<DownloadButtonProps> = (props) => {
  const { text = '下载', path, params = {}, type, downloadCenter = false, showIcon = true } = props;
  ...
  return (
    type === 'text' ?
    <a className='downloadParabola' key='download' onClick={() => download()}>{ showIcon ? <DownloadOutlined /> : null } { text }</a> :
    <Button disabled={disabled||props.disabled} className='downloadParabola' key='download' icon={showIcon ? <DownloadOutlined /> : null} onClick={() => download()}>{ text }</Button>
  )
}
