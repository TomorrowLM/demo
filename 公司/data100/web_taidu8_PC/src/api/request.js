...
export default function request(url, params) {
    ...
    newParams.terminalCode = "3"
    newParams.sign = methods.exchangeMD5(Object.assign(newParams, { nonce_str: methods.nonce_str() }))
    var method = 'post'
    $loading.show()
    // if (url == urls.is_can_answer || url == urls.editUserFullInfo || url == urls.commit_before_question) {
    //     $loading.show()
    // }
    ...
    return new Promise((resolve, reject) => {
        axios[method](url, newParams)
            .then(response => {
                ...
                if (response.data.code == 1 || (response.data.code == 0 && url == urls.smsCode) || (response.data.code == 0 && url == urls.verifyPhoneBind)) {//0代表验证码请求限制
