<template>
  <div class="wallet">
    <div class="balance">
      <img src="../../static/images/wallet_ic_rem.png" alt />
      <div class="info">
        <p>
          钱包余额：
          <span>{{userInfo.balance}}</span>元
        </p>
        <p>若要提现，请下载app提现 (右侧扫码下载)</p>
      </div>
      <div class="download">
        <ul>
          <li>
    ...
    <div class="details">
      <div class="title">
        <ul>
          <li :class="[tabIndex==0?'active':'']" @click="changeTab(0)">收入明细</li>
          <li :class="[tabIndex==1?'active':'']" @click="changeTab(1)">转出明细</li>
        </ul>
        <div class="sum">
          <img src="../../static/images/wallet_ic_total2.png" alt />
          <p v-show="tabIndex==0">
            累计收入：
            <span>{{userInfo.total}}</span>元
          </p>
          <p v-show="tabIndex==1">
            累计转出：
            <span>{{userInfo.withdraw}}</span>元
          </p>
        </div>
      </div>
