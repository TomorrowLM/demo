  ...
  <div class="detailList">
    ...
    <div v-if="!isEmpty">
      <ul v-for="(item,index) in items" :item="item" :key="index">
        <li>{{item.title}}</li>
        <li>{{item.date}}</li>
        <li class="down">-{{item.gold}}</li>
      </ul>
      <!-- <detail-list v-for="(item,index) in items" :item="item" :key="index" pageName="out"></detail-list> -->
...
export default {
  ...
  methods: {
    ...
    getData() {
      return common
        .getUserGoldExpensesRecord(String(this.page), String(this.pageSize))
        .then(data => {
